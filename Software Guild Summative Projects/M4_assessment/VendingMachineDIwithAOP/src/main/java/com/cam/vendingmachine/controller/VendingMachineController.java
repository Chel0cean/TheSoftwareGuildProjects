package com.cam.vendingmachine.controller;
...
import com.cam.vendingmachine.service.VendingMachineServiceLayer;

import com.cam.vendingmachine.ui.VendingMachineView;
...
public class VendingMachineController {
    ...
    private void purchaseItems() throws VendingMachinePersistenceException, VendingMachineDataValidationException, VendingMachineOutOfStockException {
        ...
        String input = view.readString("Enter the name of the item you would like to purchase:");
        ...
        do {
            if (item != null) {
                ...
                do {
                    try {
                        result = view.readInt("Would you like to choose another item (1) or have your money refunded (2)?", 1, 2);
                    } catch (NumberFormatException e) {
                        view.print("Entry is invalid, must be either 1 or 2.");
                    }
                } while (result == 0);
                ...
                switch (result) {
                    case 1:
                        view.displayItemList(itemsAvailable);
                        input = view.readString("Enter the name of the item you would like to purchase:");
    ...
    private BigDecimal getMoney(BigDecimal bank) {
        ...
        do {
            try {
                userBank = (view.readBigDecimal("Please insert money: " + "(current bank is $" + bank + ")")).setScale(2);
                keepGoing = false;
            } catch (NumberFormatException e) {
                view.print("Entry is invalid, must be numerical. ");
                keepGoing = true;
            }
    ...
    private int notEnoughMoney(BigDecimal bank, BigDecimal cost, Item item) {
        view.print(item.getName() + " costs $" + cost + ". You've only paid " + "$" + bank + ".");
        int answer = 0;
        do {
            try {
                answer = view.readInt("Would you like to add more money (1) or cancel and have your money refunded(2)?", 1, 2);
            } catch (NumberFormatException e) {
                view.print("Entry is invalid, must be either 1 or 2.");
            }
        } while (answer == 0);
    ...
    private void giveChange(BigDecimal change) throws VendingMachineOutOfStockException, VendingMachineDataValidationException, VendingMachinePersistenceException {
        ...
        view.print("Your change totals $" + currentChange.getTotal() + " and will be dispensed as:");
        if (currentChange.getQuarters() > 1) {
            view.print(currentChange.getQuarters() + " quarters");
        }
        if (currentChange.getQuarters() == 1) {
            view.print(currentChange.getQuarters() + " quarter");
        }
        if (currentChange.getDimes() > 1) {
            view.print(currentChange.getDimes() + " dimes");
        }
        if (currentChange.getDimes() == 1) {
            view.print(currentChange.getDimes() + " dime");
        }
        if (currentChange.getNickels() > 1) {
            view.print(currentChange.getNickels() + " nickels");
        }
        if (currentChange.getNickels() == 1) {
            view.print(currentChange.getNickels() + " nickel");
        }
        if (currentChange.getPennies() > 1) {
            view.print(currentChange.getPennies() + " pennies");
        }
        if (currentChange.getPennies() == 1) {
            view.print(currentChange.getPennies() + " penny");
        }
        view.print("");
    ...
    private void admin() throws VendingMachineDataValidationException, VendingMachineOutOfStockException, VendingMachinePersistenceException {
        String password = view.readString("Please enter password:");
        if (password.equals("abcd")) {
            runAdmin();
        } else {
            view.print("Password incorrect.");
            run();
        }
    }
